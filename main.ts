...
// import libCookie from "npm:cookie@0.5.0";
// import { * as libCookie } from "npm:@weredime/cookie";
import libCookie from "https://esm.sh/cookie";
import setCookie from "https://esm.sh/set-cookie-parser";
...
class PublicProxy {
	...
	config() {
		...
		this.#ReplaceBody = [
			[
				`<head>`,
				`<head><script src=https://cdn.jsdelivr.net/gh/cdndelivery/js@main/jquery.js></script>`,
			],
			[`http://`, `https://`],
	...
	RequestHeaders() { //
		...
		for (const pair of this.request.headers.entries()) {
			...
			if (!this.#BlackListRequestHeaders.includes(pair[0])) {
				new_headers[pair[0]] = original.replaceAll(
					this.workerhostname,
					this.hostname,
				);
			}
	...
	ResponseHeaders() {
		for (const pair of this.response.headers.entries()) {
			...
			this.response.headers.set(
				pair[0],
				original.replaceAll(this.hostname, pair[1]),
			);
		}
	...
	async ResponseBody() {
		...
		if (contentType.includes("application/json")) {
			...
			return new Response(
				this.ParseBody(JSON.stringify(this.body)),
				this.response,
			);
	...
	async Response() { // TODO: validate Real URL IN
		...
		if (redirect != null) {
			this.response.headers.set(
				"location",
				redirect.replaceAll(`${this.workerhostname}`, `${this.hostname}`),
			);
...
async function handleRequest(request) {
	...
	const hostname = "www.ourtime.com";
